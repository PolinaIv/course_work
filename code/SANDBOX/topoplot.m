...
BLANKINGRINGWIDTH = .065;% width of the blanking ring 
 ...
 if ~strcmpi(STYLE,'grid') %  not a rectangular channel grid
  ...
  elseif strcmp(STYLE,'both')  % plot interpolated surface and surface contours
      if strcmp(SHADING,'interp')
       ...
       tmph = imagesc( Xi(1,:)'-delta/2,Yi(:,1)-delta/2,Zi);
...
rin    =  0.97*rmax*(1-rwidth/2);              % inner ring radius
if hin>rin
...
if strcmp(CONVHULL,'on') %%%%%%%%% mask outside the convex hull of the electrodes %%%%%%%%%
  ...
  xx = xx*1.05; yy = yy*1.05;           % extend spline outside electrode marks
...
axis equal;
set(gca, 'xlim', [-0.625 0.625]); set(plotax, 'xlim', [-0.625 0.625]);
set(gca, 'ylim', [-0.625 0.625]); set(plotax, 'ylim', [-0.625 0.625]);
