function [ Res A] = PairwiseClassification3(data_state,TrainingFraction,Dur,StepRange,Ncomp)
    ...
    for i = 1:Nc
       for j = 1:Nc
           if(i==j) continue; end;
            ...
            range = 1:Dur;
            t = 1;
            while(range(end)<size(data_1_train,2))
                 R = data_1_train(:,range)*data_1_train(:,range)'/length(range);
                 R1(:,t) = R(LTIndex);
            ...
            range = 1:Dur;
            t = 1;
            while( range(end) < size(data_2_train,2))
                 R = data_2_train(:,range)*data_2_train(:,range)'/length(range);
                 R2(:,t) = R(LTIndex);
                 range = range+StepRange;
                 t = t+1;
            end;
