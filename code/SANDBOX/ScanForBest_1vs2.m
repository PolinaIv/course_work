...
for Fc_high = 2:15
    ...
    for Bw = 3:8
        ...
        ind1 = find(states_cur ==2);
        ind2 = find(states_cur ==1);
        
        indTr1 = ind1(1:fix(length(ind1)/2));
        indTr2 = ind2(1:fix(length(ind2)/2));

       % indTr1 = ind1(1:fix(length(ind1)));
       % indTr2 = ind2(1:fix(length(ind2)));

        indTst1 = ind1(fix(length(ind1)/2)+1:end);
        indTst2 = ind2(fix(length(ind2)/2)+1:end);

        for i=1:10
            Result{i}= ClassifyPair(data_cur, indTr1,indTr2,0.1,200,3);
            Acc(i) = Result{i}.Acc;
        end;

        [accMax, indMax] = max(Acc);
        Res(h,l) = accMax;
        ResultTest = TestPair(data_cur, indTst1,indTst2,Result{indMax}, 200,3);
...
for i=1:size( ResHL{9,1}.G12,1) 
    topoplot( ResHL{9,1}.G12(i,:),a.chanlocs_vis,'electrodes','labelpoint','chaninfo',EEGDummy.chaninfo);
    pause;
    hold off;
end;
